...
def main():
    ...
    consultaSQL = """
                    SELECT instancia, CASE WHEN nota >= 4 then 'aprobo' ELSE 'no aprobo' END as estado, count(*) as cantidad
                    from examen
    ...
    consultaSQL = """
                    SELECT DISTINCT Instancia, Nota, AVG(nota) as promedio
                    FROM examen03
                    GROUP BY Instancia, nota
                  """
    ...
    consultaSQL = """
                SELECT est.*, e.Nota as nota_parcial_01
                FROM datosEstudiantes est
                INNER JOIN examen e
                ON est.Nombre = e.Nombre
    ...
    consultaSQL = """
                SELECT est.*, e.Nota as nota_parcial_2
                FROM datosHastaParcial01 as est
                LEFT OUTER JOIN examen e
                ON est.Nombre = e.Nombre AND e.Instancia = 'Parcial-02'
                  """
    ...
    consultaSQL = """
                SELECT est.*, e.Nota as nota_recu_01
                FROM datosHastaParcial02 est
                LEFT OUTER JOIN examen e
                ON est.Nombre = e.Nombre AND e.Instancia = 'Recuperatorio-01'
                
                  """
    datosHastaRecuperatorio01 = sql^ consultaSQL
    datosHastaRecuperatorio01
    
    # ... Paso 5: Agregamos las notas del Recuperatorio-02
    consultaSQL = """
                SELECT est.*, e.Nota as nota_recu_02
                FROM datosHastaRecuperatorio01 as est
                LEFT OUTER JOIN examen e
                ON est.Nombre = e.Nombre AND e.Instancia = 'Recuperatorio-02'
                  """

    datosHastaRecuperatorio02 = sql^ consultaSQL
    datosHastaRecuperatorio02
    ...
    consultaSQL = """
                    SELECT DISTINCT datos.*, CASE WHEN (datos.nota_parcial_01 >= 4 OR datos.nota_recu_01 >= 4) AND (datos.nota_parcial_2 >= 4 OR datos.nota_recu_02 >= 4) THEN 'APROBO' ELSE 'NO APROBO' END as estado
                    FROM datosHastaRecuperatorio02 as datos
                  """
